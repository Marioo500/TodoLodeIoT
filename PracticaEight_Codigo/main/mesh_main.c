...
static const char *TAG_EJEMPLO = "ESTO ES LO QUE RECIBE";
...
void leer_sensor()
{
    ...
    snprintf(buffer, 50, "Medicion: %f", voltage);
    
}

void calcular_mediciones()
{
    float mediana,prom, varianza;
    mediana = voltage_mediana();
    prom = voltage_media();
    varianza = voltage_varianza();
    snprintf(buffer_mediciones, 50, "<%f>,<%f>,<%f>", prom, mediana,varianza);
}
...
void esp_mesh_p2p_tx_main(void *arg)
{
    ...
    while (is_running) 
    {
        ...
        if (!esp_mesh_is_root())
        {
            ...
            snprintf( (char*)tx_buf, TX_SIZE, "%s:%s", buffer, buffer_mediciones); 
    ...
    while (is_running) 
    {
        ...
        if (data.size) 
        {
            if (esp_mesh_is_root())
            {
                ...
                {
                    snprintf(MAC_FROM, 18, ""MACSTR"", MAC2STR(from.addr));
                    snprintf(MAC_ROUTE, 18, ""MACSTR"", MAC2STR(route_table[i].addr));
                    if (strcmp(MAC_FROM, MAC_ROUTE))
                    {
